...
void R_InitPlanes (void)
{
	...
	spanstop=malloc(ScreenHeight*sizeof(*spanstop));
	MaxOpenings=ScreenWidth*64;
	openings=malloc(MaxOpenings*sizeof(*openings));
	visplanes=malloc(MAXVISPLANES*sizeof(*visplanes));
	memset(visplanes, 0, MAXVISPLANES*sizeof(*visplanes));
	for (pl=visplanes;pl<&visplanes[MAXVISPLANES];pl++)
	{
		pl->ptop=malloc((ScreenWidth+2)*sizeof(*pl->ptop));
		pl->pbottom=malloc((ScreenWidth+2)*sizeof(*pl->pbottom));
		pl->top=&pl->ptop[1];
		pl->bottom=&pl->pbottom[1];
	}
  // Doh!
}
  ...
  int           lightlevel )
{
	...
	memset (check->top,0xff,ScreenWidth*sizeof(*check->top));
  ...
  int           stop )
{
	...
	memset (pl->top,0xff,ScreenWidth*sizeof(*pl->top));
