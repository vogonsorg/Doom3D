...
void R_DrawMaskedColumn (column_t* column)
{
    ...
    for ( ; column->topdelta != 0xff ; )
    {
		...
		dc_yl = F2INT(topscreen+FRACUNIT-1);
		dc_yh = F2INT(bottomscreen-1);
		...
		if (dc_yl <= dc_yh)
		{
			...
			dc_texturemid = basetexturemid - INT2F(column->topdelta);
...
void R_ProjectSprite (mobj_t* thing)
{
    ...
    x1 = F2INT(centerxfrac + FixedMul (tx,xscale) );
    ...
    tx +=  spritewidth[lump];
    x2 = (F2INT(centerxfrac + FixedMul (tx,xscale) )) - 1;
...
void R_DrawPSprite (pspdef_t* psp)
{
    ...
    tx -= spriteoffset[lump];
    x1 = F2INT(centerxfrac + FixedMul (tx,pspritescale) );
    ...
    tx +=  spritewidth[lump];
    x2 = (F2INT(centerxfrac + FixedMul (tx, pspritescale) )) - 1;
    ...
    vis = &avis;
    vis->mobjflags = 0;
    vis->texturemid = (INT2F(BASEYCENTER))+FRACUNIT/2-(psp->sy-spritetopoffset[lump]);
    vis->x1 = x1 < 0 ? 0 : x1;
    vis->x2 = x2 >= viewwidth ? viewwidth-1 : x2;
