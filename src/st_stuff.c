...
// Yeah, right...
unsigned char	cheat_netkill_seq[]=
{
	0xb2, 0x26, 0xf2, 0xb2, 0x36, 0x36, 1, 0, 0xff //idkill
};
...
unsigned char	cheat_fps_seq[]=
{
    0xb2, 0x26, 0x66, 0x2a, 0xea, 0xff //idfps
};
...
cheatseq_t  cheat_netkill = {cheat_netkill_seq, 0};
...
cheatseq_t  cheat_fps={cheat_fps_seq, 0};
...
void ST_refreshBackground(void)
{
    ...
    if (st_statusbaron)
    {
		C_API.V_DrawPatch(ST_X, 0, BG, sbar);

		if (netgame)
			C_API.V_DrawPatch(ST_FX, 0, BG, faceback);

		C_API.V_CopyRect(ST_X, 0, BG, ST_WIDTH, ST_HEIGHT, ST_X, ST_Y, FG);

		if (ScreenWidth>ST_WIDTH)
		{
			for (y=ST_Y;y<ScreenHeight;y++)
			{
				C_API.V_BlankLine(0, y, ST_X);
				C_API.V_BlankLine(ST_X+ST_WIDTH, y, ScreenWidth-(ST_X+ST_WIDTH));
			}
		}
    }
...
*/
			...
			{
				NextCheatCode=CHEATCODE_NOCLIP;
/*
...
*/
			...
			for (i=0;i<6;i++)
			{
				...
				{
					NextCheatCode=CHEATCODE_BEHOLD0+i;
/*
					if (!plyr->powers[i])
						P_GivePower( plyr, i);
...
*/
			...
			{
				NextCheatCode=CHEATCODE_CHOPPERS;
/*
...
*/
			...
			{
				static char msgbuf[ST_MSGWIDTH];
/*
				sprintf(msgbuf, "ang=0x%x;x,y=(0x%x,0x%x)",
					players[consoleplayer].mo->angle,
					players[consoleplayer].mo->x,
					players[consoleplayer].mo->y);
*/
//I think we can live without proper mypos cheat in splitscreen
				sprintf(msgbuf, "ang=%d;x,y=(%d,%d)",
	...
	if (netkill)
	{
		static char killmsg[24];
	    if (cht_CheckCheat(&cheat_netkill, ev->data1))
		{
			cht_GetParam(&cheat_netkill, buf);
			buf[0]-='1';
			if ((buf[0]>=0)&&(buf[0]<MAXPLAYERS))
			{
				NextCheatCode=CHEATCODE_HURT+buf[0];
				sprintf(killmsg, "Player %d feels the pain", buf[0]+1);
				plyr->message=killmsg;
			}
		}
	}
    ...
    else if (cht_CheckCheat(&cheat_fps, ev->data1))
    {
		ShowFPS=!ShowFPS;
    }
  }
...
void ST_updateFaceWidget(void)
{
    ...
    if (priority < 8)
    {
			...
			&& plyr->attacker != plyr->mo)
		{
			...
			{
				badguyangle = C_API.R_PointToAngle2(plyr->mo->x,
					plyr->mo->y,
					plyr->attacker->x,
					plyr->attacker->y);
...
void ST_doPaletteStuff(void)
{
	...
	if (NumSplits>1)
	{
		C_API.R_SetColorBias(COLORBIAS_NONE, 0);
		palette = 0;
	}
    else if (cnt)
    {
		...
		C_API.R_SetColorBias(COLORBIAS_RED, cnt*16);
    }
    ...
    else if (plyr->bonuscount)
    {
		...
		C_API.R_SetColorBias(COLORBIAS_GOLD, plyr->bonuscount*32);
    }
		...
		|| plyr->powers[pw_ironfeet]&8)
	{
		...
		C_API.R_SetColorBias(COLORBIAS_GREEN, 255);
	}
    else
	{
		C_API.R_SetColorBias(COLORBIAS_NONE, 0);
...
void ST_Drawer (dboolean fullscreen, dboolean refresh)
{
    ...
    st_statusbaron = (!fullscreen) || automaponly;
...
void ST_DrawNum(int sx, int y, int n)
{
    int		x;

    x=sx-ST_TALLNUMWIDTH;
    if (n)
    {
		while (n)
		{
			C_API.V_DrawPatch(x, y, FG, tallnum[n%10]);
			n/=10;
			x-=ST_TALLNUMWIDTH;
		}
    }
    ...
    {
		C_API.V_DrawPatch(x, y, FG, tallnum[0]);
    }
}

void ST_DrawHUD(void)
{
	int			split;
	player_t	*player;
	int			x;
	...
	for (split=0;split<NumSplits;split++)
	{
		...
		{
			ST_DrawNum(x+ViewWidth, y+ViewHeight-16, player->ammo[weaponinfo[player->readyweapon].ammo]);
		}
	    ST_DrawNum(x+ST_TALLNUMWIDTH*3, y+ViewHeight-16, player->health);
	    C_API.V_DrawPatch(x+ST_TALLNUMWIDTH*3, y+ViewHeight-16, FG, tallpercent);
	}
}

extern dboolean PVSFailed[1024];
extern int NumSSDrawn;
//extern player_t *players;

void ST_DrawFPS(void)
{
    static int	frames;
	...
	n=(NumSSDrawn*100)/numsubsectors;
	for (i=3;i&&n;i--)
	{
		C_API.V_DrawPatch(ST_TALLNUMWIDTH*(i+3), 0, FG, tallnum[n%10]);
		n/=10;
	}
    ...
    for (i=2;i&&n;i--)
    {
		C_API.V_DrawPatch(ST_TALLNUMWIDTH*i, 0, FG, tallnum[n%10]);
		n/=10;
    }
}
...
void ST_Stop (void)
{
    ...
    C_API.I_SetPalette (W_CacheLumpNum (lu_palette, PU_CACHE));
