...
void P_DeathThink (player_t* player)
{
	...
	{
		angle = C_API.R_PointToAngle2 (player->mo->x,
								 player->mo->y,
								 player->attacker->x,
								 player->attacker->y);
...
void P_AdvanceWeapon(player_t *player)
{
...
void P_PlayerThink (player_t* player)
{
	...
	if (player->powers[pw_invulnerability])
	{
		if (player->powers[pw_invulnerability] > 4*32
			...
			player->fixedcolormap = FCM_INVUL;
		else
			player->fixedcolormap = FCM_NONE;
	}
	else if (player->powers[pw_infrared])
	{
		...
		{
			player->fixedcolormap = FCM_FULLBRIGHT;
		}
		else
			player->fixedcolormap = FCM_NONE;
	}
