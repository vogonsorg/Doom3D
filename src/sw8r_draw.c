...
//FUDGE: doesn't work right if NumColorMaps!=32
 ...
 int		count )
{
	...
	//  at one point.
    memcpy ((byte *)screens[0]+ofs, (byte *)screens[1]+ofs, PIX2BYTE(count));
}
...
void R_MergeMatrixViews(void)
{
	...
	dest=screens[0]+viewwindowx+viewwindowy*ScreenWidth;
